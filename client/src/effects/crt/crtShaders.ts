...
void main(){
 ...
 float edgeFade = length(uCurve) > 0.001
   ? smoothstep(0.0, 0.020, min(ed.x, ed.y))
   : 1.0;
 inside *= edgeFade;
 ...
 /* soft rolling band + thin refresh line sweeping downward */
 float bar = fract(uv.y*0.5 - t*0.08*uMotion);
 bar = smoothstep(0.0,0.05,bar)*smoothstep(0.18,0.05,bar);
 col += bar*uBar*uMotion;
 float sweep = fract(uv.y - t*0.13*uMotion);
 float scanBeam = smoothstep(0.016,0.0,sweep)*smoothstep(0.032,0.008,sweep);
 col += scanBeam*uBar*0.9*uMotion;
 col *= 1.0 - scanBeam*uBar*0.14;
